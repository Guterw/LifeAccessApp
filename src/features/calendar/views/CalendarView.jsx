...
import {
  ...
  Trash2, Pencil, CalendarDays, PartyPopper
} from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-2 leading-relaxed">
              {type === 'since'
                ? t('calendar.typeSinceDesc', 'Conta quantos dias já passaram desde a data escolhida (ex: dias sem açúcar).')
                : t('calendar.typeUntilDesc', 'Conta quantos dias faltam até a data escolhida (ex: faltam X dias para a viagem).')}
            </p>
          </div>
...
// ==========================================
// CARD DE CONTADOR (usado no menu lateral)
// ==========================================
...
export default function CalendarView() {
  ...
  // Mapa: dateKey -> array de contadores ancorados naquele dia
    ...
    <div className="w-full pt-8 animate-fade-in pb-24 px-4 -mt-5 -mb-20 min-h-screen">
      ...
      <div className="flex items-center gap-3 mb-6 -mt-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 font-bold uppercase tracking-wide">{t('calendar.subtitle', 'Contadores e Datas Importantes')}</p>
        </div>
      </div>
        ...
        {monthCells.map((cell) => {
          ...
          return (
            ...
            >
              ...
              {/* Badges dos contadores ancorados neste dia */}
              {dayCounters.length > 0 && (
                <div className="flex gap-0.5 mt-0.5 absolute bottom-1">
                  {dayCounters.slice(0, 3).map((c) => {
                    const cfg = getColorConfig(c.color);
                    return <div key={c.id} className={`w-1.5 h-1.5 rounded-full ${cfg.bg}`} />;
                  })}
                </div>
              )}
            </button>
      ...
      <div className="bg-gray-800/60 rounded-2xl border border-gray-700 p-4 mb-6">
        ...
        {selectedDayCounters.length === 0 ? (
          <p className="text-sm text-gray-500">{t('calendar.noCountersOnDay', 'Nenhum contador ancorado neste dia.')}</p>
        ) : (
          <div className="space-y-2">
            {selectedDayCounters.map((c) => (
              <div key={c.id} className="flex items-center justify-between bg-gray-900/60 rounded-xl p-2.5">
                ...
                <button
                  onClick={() => setModalState({ open: true, data: c })}
      ...
      {/* BOTÃO FLUTUANTE: NOVO CONTADOR */}
      <button
        onClick={() => setModalState({ open: true, data: null })}
        className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg transition-all active:scale-[0.98]"
      >
        <Plus size={20} />
      ...
      {/* ========================================== */}
      {/* MENU LATERAL (DRAWER) COM TODOS OS CONTADORES */}
      {/* ========================================== */}
      {isMenuOpen && (
          ...
          <div className="w-[85%] max-w-sm bg-gray-950 border-l border-gray-800 h-full overflow-y-auto shadow-2xl animate-slide-in">
            <div className="sticky top-0 bg-gray-950 border-b border-gray-800 p-5 flex items-center justify-between z-10">
              <h3 className="text-lg font-black text-white flex items-center gap-2">
                ...
                {t('calendar.myCounters', 'Meus Contadores')}
              </h3>
            ...
            <div className="p-4 space-y-3">
              <button
                onClick={() => setModalState({ open: true, data: null })}
                className="w-full bg-purple-600/20 border-2 border-dashed border-purple-500/40 text-purple-400 rounded-2xl p-4 flex items-center justify-center gap-2 font-bold text-sm hover:bg-purple-600/30 transition-colors"
              >
                <Plus size={18} />
                {t('calendar.newCounter', 'Novo Contador')}
              </button>

              {counters.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  <Hourglass size={40} className="mx-auto mb-3 opacity-40" />
                  <p className="text-sm font-medium">{t('calendar.emptyCounters', 'Nenhum contador criado ainda.')}</p>
                  <p className="text-xs mt-1 opacity-70">{t('calendar.emptyCountersDesc', 'Ex: dias sem álcool, dias sem açúcar, dias em Dublin, ou faltam X dias para algo.')}</p>
                </div>
              ) : (
                counters
