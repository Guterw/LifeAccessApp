// src/features/languages/english/views/vocabularies/vocab-speech/LevelGroupView.jsx
import React, { useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { vocabSpeechLevels } from '../../../../../../data/vocabSpeechLevels';
import { useLanguage } from '../../../../../../contexts/LanguageContext';
import BackButton from '../../../../../../components/BackButton';
import { Mic, CheckCircle2, Flame, RotateCcw, AlertTriangle } from 'lucide-react';
...
export default function LevelGroupView() {
  ...
  const completedLevels = useLiveQuery(() => db.completedLevelsSpeech.toArray(), []) || [];
  const levelsProgress = useLiveQuery(() => db.levelProgressSpeech.toArray(), []) || [];
  ...
  const levelsInGroup = Object.values(vocabSpeechLevels).filter((l) => {
  ...
  const confirmRestart = async () => {
    ...
    await db.completedLevelsSpeech.delete(lvlId);
    await db.levelProgressSpeech.delete(lvlId);
    ...
    <div className="fixed inset-x-0 top-0 bottom-[80px] bg-gray-950 flex flex-col animate-fade-in z-10 overflow-hidden">
      <div className="shrink-0 h-16 w-full bg-gray-900 border-b border-gray-800 z-20 flex items-center justify-between px-4 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="shrink-0 mt-9 mr-3">
            <BackButton to="/english/vocabularies/vocab-speech/levels" label="" />
          </div>
          <div className="w-9 h-9 bg-pink-500/10 text-pink-400 rounded-xl border border-pink-500/20 shadow-inner flex items-center justify-center shrink-0 mr-3">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        {levelsInGroup.map((level) => {
          ...
          const barColor = done ? 'bg-green-500' : 'bg-pink-500';

          return (
            <button
              key={level.id}
              onClick={() => navigate(`/english/vocabularies/vocab-speech/level/${level.id}`)}
              className={`w-full text-left p-6 rounded-2xl border block transition-all shadow-lg ${
                done ? 'bg-gray-800/50 border-green-500/30' : 'bg-gray-800 border-gray-700 hover:border-pink-500'
              }`}
            >
              <div className="flex items-center justify-between mb-4 gap-4">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {done ? (
                    ...
                    <Mic className="text-pink-500 shrink-0" size={32} />
                  )}
                </div>
              </div>
