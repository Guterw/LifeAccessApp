...
export default function SyncChoiceView({ onOffline }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      alert(t('syncChoice.loginError', 'Erro ao conectar com o Google. Tente novamente.') + detail);
      setIsLoading(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-sm relative z-10 space-y-8 mt-4">
        ...
        <div className="w-full space-y-4 pt-4">
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full bg-white hover:bg-gray-100 text-gray-900 p-4 rounded-2xl flex items-center justify-center gap-3 font-black shadow-lg transition-transform active:scale-95 disabled:opacity-60"
          >
            {isLoading ? (
            ...
            {isLoading ? t('syncChoice.connecting', 'Conectando...') : t('syncChoice.googleBtn', 'Entrar com Google')}
          </button>
          ...
          <button 
            onClick={onOffline}
            disabled={isLoading}
            className="w-full bg-gray-800 border-2 border-gray-700 hover:border-gray-600 text-white p-4 rounded-2xl flex flex-col items-center justify-center transition-colors active:scale-95 disabled:opacity-60"
          >
